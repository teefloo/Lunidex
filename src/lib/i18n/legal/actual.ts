...
const updatedEn = 'September 7, 2026';
  ...
  privacy: {
    ...
    sections: [
      ...
      { id: 'bases', title: '2. Finalités et bases légales', table: { headers: ['Traitement', 'Base légale'], rows: [
        ...
        ['Mesure d’audience, performance Sentry et mesure produit Neon', 'Consentement préalable et distinct pour les mesures facultatives, retirable à tout moment (article 6.1.a du RGPD).'],
      ...
      { id: 'rights', title: '7. Vos droits et contact', paragraphs: [
        ...
        'Vous pouvez refuser ou retirer séparément les mesures Vercel, Sentry et Neon depuis le bandeau de consentement ou le bouton « Gérer mes préférences » du pied de page. Le retrait empêche les intégrations concernées de se charger et supprime les marqueurs de mesure locaux prévus par l’application.',
...
const en: LegalDocuments = {
  privacy: {
    ...
    sections: [
      ...
      { id: 'measurement', title: '4. Measurement, logs and technical data', paragraphs: ['Vercel Web Analytics, Speed Insights and Sentry browser tracing or Session Replay load only after your separate audience and performance consent. Replay masks text and inputs and blocks media. Neon product measurement is also optional: when allowed, Lunidex sends only predefined TCG events and limited properties to produce aggregated daily counters retained for 90 days. These events are not joined to user_state or your account.', 'Lunidex also uses Sentry for error and availability monitoring. The SDK does not send personal data by default, and Lunidex removes URL query parameters, cookies, headers and request bodies before sending. Infrastructure and abuse protection may process technical request information such as IP address, URL, timestamp and response code. The contact route validates, rate-limits and forwards messages through Resend but does not store or log message content in Lunidex.'] },
      { id: 'recipients', title: '5. Services, recipients and transfers', paragraphs: ['Neon provides authentication and application data when those variables are configured. Vercel hosts and distributes the application. Sentry receives technical error events and, with consent, performance and privacy-protected replay data. Resend receives the information required to send contact messages (email address, name, subject and content). PokéAPI, TCGdex and image hosts provide public data requested by the application. Each provider may process technical data under its own terms.', 'Resend, Sentry and Vercel may process data in the United States or other countries according to their configuration. Transfers rely on the safeguards published by the providers, including applicable data processing agreements and transfer mechanisms. See https://resend.com/legal/dpa, https://sentry.io/legal/dpa/ and https://vercel.com/legal/dpa.', 'Lunidex does not deploy advertising pixels, sell data or create advertising profiles. Authentication cookies are managed by Neon Auth; their name and lifetime may depend on the provider configuration.'] },
  ...
  cookies: {
    ...
    sections: [
      ...
      { id: 'measurement', title: '3. Optional measurement', paragraphs: ['Vercel Web Analytics, Speed Insights and Sentry browser tracing or Session Replay load only if you grant the audience and performance choice. Replay masks text and inputs and blocks media. Neon receives only limited, aggregated TCG product measurement if you grant the separate product measurement choice. Neither choice is required for public pages.'] },
