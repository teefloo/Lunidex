...
async function patchTransaction(request: NextRequest, context: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  if (!rateLimit(`tcg-sealed-update:${auth.userId}`, 30)) return NextResponse.json({ error: 'Too many sealed portfolio requests.' }, { status: 429, headers: { 'Cache-Control': 'private, no-store' } });
  ...
  try {
    const result = await mutateSealedTransaction(auth.sql, auth.userId, body, { mode: 'update', id, transactionRevision: body.revision, expectedRevision: expectedRevision as number | undefined });
...
export const PATCH = withObservedRouteHandler('/api/tcg/sealed/transactions/:id', 'tcg-sealed', patchTransaction);
