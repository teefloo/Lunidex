...
async function removeApplicationData(sql: NeonSql, userId: string): Promise<void> {
  await sql.transaction((tx) => [
    ...
    tx`delete from public.tcg_sealed_portfolio_daily where user_id = ${userId}::uuid`,
    tx`delete from public.tcg_sealed_audit where user_id = ${userId}::uuid`,
    tx`delete from public.tcg_sealed_allocations where user_id = ${userId}::uuid`,
    tx`delete from public.tcg_sealed_aliases where user_id = ${userId}::uuid`,
    tx`delete from public.tcg_sealed_transactions where user_id = ${userId}::uuid`,
    tx`delete from public.tcg_sealed_user_revisions where user_id = ${userId}::uuid`,
