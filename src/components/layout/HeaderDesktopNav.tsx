...
export function HeaderDesktopNav() {
  ...
  return (
    <nav className="hidden min-w-0 flex-none items-center justify-center gap-0 rounded-sm border border-border bg-background/40 px-1 py-0.5 lg:flex">
      ...
      <DropdownMenu>
        <DropdownMenuTrigger className="group inline-flex items-center justify-center gap-1 whitespace-nowrap rounded-sm border border-transparent px-1.5 py-1 text-[9px] font-bold uppercase tracking-[0.16em] text-muted-foreground transition-all duration-100 hover:border-border/60 hover:bg-muted/70 hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring data-[popup-open]:text-primary">
          {label('nav.more', 'More')}
        ...
        <DropdownMenuContent align="start">
          {SECONDARY_NAV_ITEMS.map((item) => (
            <DropdownMenuItem
              key={item.path}
              render={
                <Link href={localizedHref(item.path)} className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.14em] text-muted-foreground hover:text-primary">
