...
export function TCGCollectionStartLink() {
  ...
  const activeSet = usePrimeDexStore((state) => state.tcgActiveSets[0]);
  const isReturning = mounted && hasHydrated && ownedCount > 0;
  const href = isReturning && activeSet ? localeHref(`/tcg/collection/${activeSet}`) : isReturning ? localeHref('/tcg/collection') : localeHref('/tcg/start');
  ...
  return (
    <Link href={href} className="inline-flex min-h-12 items-center gap-2 rounded-sm border border-primary/45 bg-primary/10 px-5 text-sm font-black uppercase tracking-[0.1em] text-primary transition-colors hover:bg-primary/15 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60">
      {isReturning ? t('tcg.activation.resume_cta', { defaultValue: 'Resume my collection' }) : t('tcg.activation.start_cta', { defaultValue: 'Start my collection' })}
