...
- `SYNCED_KEYS` is the contract shared by local persistence and `src/lib/supabase/sync-state.ts`. Add a new synchronized field deliberately and keep its default, hydration, and server representation aligned.
...
- Treat authentication and cloud sync as best-effort. The web app must remain usable with no Supabase variables and local IndexedDB only.
