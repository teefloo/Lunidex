...
- Keep imports and APIs Deno-compatible. The function uses the Supabase JS client through a remote ESM import and its tests use `Deno.test`; do not add Node-only or npm-only assumptions.
...
The root Vitest configuration excludes this directory because these tests are Deno tests. Run the focused test with the Deno runner when Deno is available:

```bash
deno test supabase/functions/poll-tcg-prices/security.test.ts
```
